...
/// In MAUI mode, connects directly to Ollama or a remote ARIA server.
/// </summary>
public class ClientLLMService
{
    ...
    private ConnectionMode _connectionMode = ConnectionMode.DirectOllama;
    ...
    /// </summary>
    public ClientLLMService(HttpClient httpClient, Microsoft.Extensions.Configuration.IConfiguration? configuration = null)
    {
        _httpClient = httpClient;
        // Default Ollama endpoint - user will configure the actual IP in Settings
        _ollamaEndpoint = "http://localhost:11434";
        _ollamaModel = "mistral"; // Popular lightweight model
        // Load API key from configuration if available
        try
        {
    ...
    public void SetOllamaConfig(string endpoint, string model, string? apiKey = null)
    {
        ...
        _ollamaEndpoint = NormalizeBaseUrl(endpoint);
        _ollamaModel = model;
    ...
    private string ResolveApiUrl()
    {
        ...
        {
            ConnectionMode.RemoteServer => string.IsNullOrWhiteSpace(_remoteServerUrl)
                ? throw new InvalidOperationException("Remote server URL is not configured. Set a valid server URL first.")
                : $"{_remoteServerUrl}/api/ollama/chat",
    ...
    /// <summary>
    /// Tests whether the remote ARIA server URL is reachable.
    /// </summary>
    public async Task<(bool Success, string Message, string? NormalizedUrl)> TestServerLinkAsync(
        string serverUrl,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(serverUrl))
            return (false, "Server URL is empty.", null);

        var normalizedUrl = NormalizeBaseUrl(serverUrl);

        try
        {
            using var request = new HttpRequestMessage(HttpMethod.Get, $"{normalizedUrl}/api/test");
            using var response = await _httpClient.SendAsync(request, cancellationToken);

            if (response.IsSuccessStatusCode)
            {
                return (true, "Server link is working.", normalizedUrl);
            }

            return (false, $"Server responded with {(int)response.StatusCode} {response.ReasonPhrase}.", normalizedUrl);
        }
        catch (Exception ex)
        {
            return (false, $"Unable to reach server: {ex.Message}", normalizedUrl);
        }
    }
